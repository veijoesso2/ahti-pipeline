...
if [ ! -f "$HOME/ahti-pipeline/sql/master_pipeline_v3.sql" ]; then
    echo "  ❌ master_pipeline_v3.sql not found!"
...
PGPASSWORD=$DB_PASS psql -h localhost -d $DB_NAME -U $DB_USER \
    ...
    -f "$HOME/ahti-pipeline/sql/master_pipeline_v3.sql" 2>&1 | tee /tmp/pipeline_v3.log
