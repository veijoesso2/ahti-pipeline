...
-- =====================================================================
-- STEP 5: LAKE CONNECTORS
-- =====================================================================
\echo '[5/12] Creating Lake Connectors...'

-- =====================================================================
-- IMPROVED LAKE CONNECTOR LOGIC
-- This replaces the lake connector section (Step 5) in master_pipeline_fixed.sql
-- =====================================================================
...
-- 2. Lakes that are well-served by rivers (>50m of river inside)
served_lakes AS (
...
valid_feeders AS (
    SELECT * FROM river_intersections WHERE inside_len <= 50 
),
...
-- 4. Cluster ports and find centers
clustered_ports AS (
    SELECT 
        lake_id,
...
-- 6. Find nearby lake pairs for portage links
lake_pairs AS (
    SELECT 
        a.id as lake_a_id,
        b.id as lake_b_id,
    ...
    WHERE a.type = 'lake' 
      AND b.type = 'lake'
      AND ST_DWithin(a.geom, b.geom, 1500)  -- Max 1.5km apart
      AND ST_Area(a.geom) > 50000  -- Significant lakes only
      AND ST_Area(b.geom) > 50000
),

-- 7. Create portage link candidates
portage_candidates AS (
    SELECT 
        ...
        -- Don't create if BOTH lakes already have rivers running through them
        CASE 
            ...
            THEN 'SKIP: Gap too large'
            ELSE 'VALID'
        END as status
    FROM lake_pairs
),

-- 8. Combine all valid connectors
all_connectors AS (
...
SELECT 
    ...
    'connector',
    CASE 
        WHEN conn_type = 'star_connector' THEN 'Star Connector (' || ROUND(length::numeric) || 'm)'
        WHEN conn_type = 'portage_link' THEN 'Portage Link (' || ROUND(length::numeric) || 'm)'
        ELSE 'Connector'
    END,
    geom, 
    TRUE 
