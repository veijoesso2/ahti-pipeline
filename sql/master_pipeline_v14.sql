-- =====================================================================
-- AHTI PADDLE MAP PIPELINE - MASTER SCRIPT v14 (CULVERT HEALING)
-- =====================================================================
-- v14 Changes:
-- 1. ACTIVATED STEP 4: "River Healing".
--    Detects gaps < 40m between river ends (culverts/tunnels) and 
--    stitches them together. This fixes discontinuities at roads.
-- 2. Preserves v13 "Anti-Loop" logic and v12.1 fixes.
-- =====================================================================

\echo '╔════════════════════════════════════════════════════════════════╗'
\echo '║  AHTI PADDLE MAP PIPELINE - MASTER SCRIPT v14                ║'
\echo '╚════════════════════════════════════════════════════════════════╝'
\echo ''
...
SELECT 
    osm_id, 'osm_line', 'river', name, way
FROM planet_osm_line
WHERE waterway IN ('river', 'stream', 'canal')
AND way && ST_MakeEnvelope(:min_x, :min_y, :max_x, :max_y, 3857)
AND (
    waterway IN ('river', 'canal')
    OR (waterway = 'stream' AND COALESCE(NULLIF(regexp_replace(width, '[^0-9.]', '', 'g'), '')::numeric, 0) >= 3)
)
AND ST_Length(way) > 50;

INSERT INTO candidate_objects (osm_id, source_type, type, name, geom)
SELECT 
    osm_id, 'osm_polygon', 'lake', name, way
...
-- =====================================================================
-- STEP 4: AGGREGATE & HEAL RIVERS (Activated in v14)
-- =====================================================================
\echo '[4/12] Healing River Gaps (Culverts)...'

-- 4A. Find Dangling Ends
-- We look for endpoints that don't touch ANY other river segment.
-- This is the same logic as "True Terminals", but applied BEFORE lake connection.
...
WHERE NOT EXISTS (
    ...
    -- Strictly check intersection (touching)
...
-- 4B. Stitch Gaps
-- If two dangles are within 40m (typical road width), connect them.
INSERT INTO candidate_objects (osm_id, source_type, type, name, geom, is_virtual)
SELECT 
    990000 + ROW_NUMBER() OVER(),
    'system',
    'river', -- Treat as river so it flows continuously
    'Culvert',
    ST_MakeLine(d1.pt, d2.pt),
    TRUE
FROM river_dangles d1
JOIN river_dangles d2 
    ON d1.id < d2.id -- Match pairs only once
    AND ST_DWithin(d1.pt, d2.pt, 40) -- 40 meter gap tolerance
WHERE NOT EXISTS (
    -- Prevent "zipping" parallel rivers: Ensure we don't cross another river
    -- (Simple heuristic: dangles must be closest neighbors)
    SELECT 1 FROM river_dangles d3
    WHERE d3.id != d1.id AND d3.id != d2.id
    AND ST_DWithin(d1.pt, d3.pt, ST_Distance(d1.pt, d2.pt))
);
...
DECLARE
    healed_count INT;
BEGIN
    SELECT COUNT(*) INTO healed_count FROM candidate_objects WHERE name = 'Culvert';
    RAISE NOTICE '  Culverts/Gaps stitched: %', healed_count;
END $$;

\echo '  ✓ Rivers healed'

-- =====================================================================
-- STEP 5: SMART LAKE CONNECTORS (Full Topology Check)
-- =====================================================================
\echo '[5/12] Creating Smart Lake Connectors...'

DELETE FROM candidate_objects WHERE is_virtual = TRUE AND type = 'connector'; -- Keep 'river' culverts!

-- 5A. STRICT TERMINAL DETECTION
-- Logic Update: A point is a "True Terminal" ONLY if it does NOT touch
-- ANY PART of another river geometry (interior or endpoint).
-- IMPORTANT: Now that we added culverts in Step 4, those gaps are closed.
-- This step will now see those "healed" rivers as continuous and ignore them.
...
SELECT 
    a.pt
FROM river_endpoints_raw a
WHERE NOT EXISTS (
    SELECT 1 
    FROM candidate_objects r
    WHERE r.type = 'river'
    AND r.id != a.id  -- Don't compare with self
    AND ST_DWithin(a.pt, r.geom, 0.5) 
);

CREATE INDEX idx_tt_pt ON true_terminals USING GIST(pt);

-- 5B. Snap True Terminals to Lakes
CREATE TEMP TABLE lake_ports AS
SELECT 
    r.id as river_id,
    l.id as lake_id,
    ST_ClosestPoint(l.geom, tt.pt) as geom
FROM candidate_objects r
JOIN true_terminals tt 
    ON ST_DWithin(ST_StartPoint(r.geom), tt.pt, 0.1) 
    OR ST_DWithin(ST_EndPoint(r.geom), tt.pt, 0.1)
JOIN candidate_objects l 
    ON l.type = 'lake' AND ST_DWithin(tt.pt, l.geom, 50)
WHERE r.type = 'river';

-- 5C. Create Lake Routes (Internal)
INSERT INTO candidate_objects (osm_id, source_type, type, name, geom, is_virtual)
SELECT 
...
WITH triangle_check AS (
    SELECT 
        p1.from_lake as A, p1.to_lake as C, p2.to_lake as B,
        p1.dist as dist_direct,
        (p2.dist + p3.dist) as dist_via_B
...
USING triangle_check tc
WHERE p.from_lake = tc.A AND p.to_lake = tc.C
AND tc.dist_via_B < (3 * tc.dist_direct);

INSERT INTO candidate_objects (osm_id, source_type, type, name, geom, is_virtual)
SELECT 
    999900 + row_number() over(),
    'system',
    'connector',
    'Portage',
    geom,
...
SELECT 
    ...
    osm_id::text,
    name,
    'lake_crossing' as type,
    (ST_Dump(ST_Force2D(geom))).geom as geom
FROM candidate_objects 
WHERE type = 'connector' AND is_virtual = TRUE
UNION ALL
-- Add the new Culverts to the network!
SELECT
    (ROW_NUMBER() OVER() + 2000000) as gid,
    osm_id::text,
    name,
    'river' as type, -- Culverts are part of the river
    (ST_Dump(ST_Force2D(geom))).geom as geom
FROM candidate_objects
WHERE name = 'Culvert';
...
WITH segment_gen AS (
    SELECT 
        ...
        CASE WHEN ST_Length(geom) > 1000 THEN CEIL(ST_Length(geom) / 1000.0)::int ELSE 1 END as num_chunks, 
        geom
    FROM paddling_network
)
SELECT 
    ROW_NUMBER() OVER() as seg_id, parent_id, osm_id, name, type, network_id,
...
UPDATE paddling_segments SET environment = CASE WHEN type = 'lake_crossing' THEN 'lake_route' ELSE 'river' END;
...
UPDATE paddling_segments SET fun_score = CASE WHEN type = 'lake_crossing' THEN 60 ELSE (CASE WHEN sinuosity > 1.2 THEN 90 ELSE 70 END + CASE WHEN land_type = 'forest' THEN 15 WHEN land_type = 'field' THEN -15 WHEN land_type = 'urban' THEN -20 ELSE 0 END + CASE WHEN has_rapids THEN 20 ELSE 0 END) END;
UPDATE paddling_segments SET fun_score = 100 WHERE fun_score > 100;
UPDATE paddling_segments SET fun_score = 0 WHERE fun_score < 0;

UPDATE paddling_segments SET feasibility_score = CASE WHEN is_official_route THEN 100 WHEN type = 'lake_crossing' THEN 80 WHEN has_poi_signal THEN 90 ELSE 50 END;
