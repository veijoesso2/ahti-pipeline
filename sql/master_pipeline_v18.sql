...
AND (
    ...
    OR (
        waterway = 'stream' 
        AND (
            -- Keep your width logic, but lower threshold to 1.5m for canoes
            COALESCE(NULLIF(regexp_replace(width, '[^0-9.]', '', 'g'), '')::numeric, 0) >= 1.5
...
FROM paddling_areas
UNION ALL
SELECT (ROW_NUMBER() OVER() + 1000000) as gid, osm_id::text, name, 
       'lake_crossing' as type, (ST_Dump(ST_Force2D(geom))).geom as geom
FROM candidate_objects 
WHERE type = 'connector' AND is_virtual = TRUE; -- This captures both Portage and Road Portage
...
WHERE network_id IN (
    ...
    FROM paddling_network 
    GROUP BY network_id 
    HAVING SUM(ST_Length(geom)) < 2000 -- Remove networks shorter than 2km total
);

\echo '  ✓ Network analysis complete'

-- =====================================================================
-- STEP 7: ROBUST SEGMENTATION
-- =====================================================================
...
WITH segment_gen AS (
    SELECT 
        ...
        type, 
        network_id, 
        -- Calculate chunks, ensuring we never divide by zero
        CASE WHEN ST_Length(geom) > 1000 THEN CEIL(ST_Length(geom) / 1000.0)::int ELSE 1 END as num_chunks,
        -- Force to 2D and Merge to ensure it is a simple LINESTRING
        ST_LineMerge(ST_Force2D(geom)) as clean_geom
    FROM paddling_network
    WHERE ST_GeometryType(geom) LIKE '%LineString%' 
      AND ST_Length(geom) > 1 -- Ignore tiny artifacts that break substrings
)
SELECT 
    ...
    type, 
    network_id, 
    -- Use LEAST/GREATEST to prevent floating point errors out of (0,1) range
    ST_LineSubstring(clean_geom, 
        GREATEST(0, (n-1)::float/num_chunks), 
        LEAST(1, n::float/num_chunks)
    ) as geom
FROM segment_gen 
CROSS JOIN LATERAL generate_series(1, num_chunks) as n
-- Final safety check to ensure geometry is valid
WHERE ST_Length(clean_geom) > 0;
...
-- Add the missing columns immediately so Step 9-11 can find them
