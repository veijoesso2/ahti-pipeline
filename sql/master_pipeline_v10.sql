...
-- =====================================================================
-- STEP 5: SMART LAKE CONNECTORS v4 (Fixed Topology)
-- =====================================================================
\echo '[5/12] Creating Smart Lake Connectors...'

DELETE FROM candidate_objects WHERE is_virtual = TRUE;

-- 1. Identify "Ports" (Where rivers touch lakes)
CREATE TEMP TABLE lake_ports AS
SELECT 
    r.id as river_id,
    l.id as lake_id,
    ST_ClosestPoint(l.geom, ST_StartPoint(r.geom)) as geom -- Snap to lake edge
FROM candidate_objects r
JOIN candidate_objects l 
    ON r.type = 'river' AND l.type = 'lake'
    AND ST_DWithin(r.geom, l.geom, 50); -- 50m tolerance

CREATE INDEX idx_ports_lake ON lake_ports(lake_id);

-- 2. Internal Lake Routes (Fixes "Floating Stars")
-- Instead of one center point, connect ports directly if possible, 
-- or connect them sequentially if the lake is simple.
...
-- Note: This connects every river mouth to every other river mouth on the SAME lake.
-- For huge lakes with 50 rivers, this creates a "web". 
-- For typical lakes with 2-3 rivers, it creates a perfect path.

-- 3. Lake-to-Lake Portages (The "400m" Rule)
...
AND ST_DWithin(l1.geom, l2.geom, 400); -- Strict 400m limit

-- 4. Prune "Triangles" (The "3x Rule")
-- If A->C exists, and A->B->C exists, and (A->B + B->C) < 3 * (A->C), delete A->C.
WITH triangle_check AS (
    SELECT 
        p1.from_lake as A,
        p1.to_lake as C,
        p2.to_lake as B, -- The intermediate lake
        p1.dist as dist_direct,
        (p2.dist + p3.dist) as dist_via_B
    FROM raw_portages p1        -- Direct A->C
    JOIN raw_portages p2 ON p1.from_lake = p2.from_lake -- A->B
    JOIN raw_portages p3 ON p2.to_lake = p3.from_lake AND p3.to_lake = p1.to_lake -- B->C
)
...
\echo '  ✓ Smart connectors created (Triangles pruned, Stars fixed)'
...
BEGIN
    ...
    RAISE NOTICE '  Star connectors: %, Portage links: %', star_count, portage_count;
END $$;

\echo '  ✓ Lake connectors created'
