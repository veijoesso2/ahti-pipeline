-- =====================================================================
-- AHTI PADDLE MAP PIPELINE - MASTER SCRIPT v15 (SMART PORTAGES)
-- =====================================================================
-- v15 Changes:
-- 1. NEW PORTAGE LOGIC:
--    - Short (<500m): Always created (Direct line).
--    - Long (500m-1000m): Created ONLY if a non-private trail/road 
--      exists nearby ("Road Portage").
-- 2. Preserves v14 "Culvert Healing" and v13 "Anti-Loop" logic.
-- =====================================================================

\echo '╔════════════════════════════════════════════════════════════════╗'
\echo '║  AHTI PADDLE MAP PIPELINE - MASTER SCRIPT v15                ║'
\echo '╚════════════════════════════════════════════════════════════════╝'
\echo ''
...
-- =====================================================================
\echo '[5/12] Creating Smart Lake Connectors (v15)...'

DELETE FROM candidate_objects WHERE is_virtual = TRUE AND type = 'connector';

-- 5A. Identify Valid Portage Trails
-- We capture roads/paths that are suitable for carrying a canoe.
-- CRITICAL: Filter out private access!
CREATE TEMP TABLE portage_trails AS
SELECT way as geom
FROM planet_osm_line
WHERE 
    -- 1. Walkable/Drivable types
    highway IN ('path', 'track', 'footway', 'cycleway', 'service', 
                'unclassified', 'residential', 'tertiary', 'secondary')
    -- 2. NOT Private
    AND (access IS NULL OR access NOT IN ('private', 'no', 'customers'))
    -- 3. NOT Driveways (usually private)
...
-- 5B. True Terminals (for internal lake routes)
...
JOIN true_terminals tt ON ST_DWithin(ST_StartPoint(r.geom), tt.pt, 0.1) OR ST_DWithin(ST_EndPoint(r.geom), tt.pt, 0.1)
...
AND ST_DWithin(l1.geom, l2.geom, 1000); -- Search up to 1000m
...
SELECT 
    *,
    CASE 
        WHEN dist <= 500 THEN 'Portage'       -- Always valid
        ELSE 'Road Portage'                   -- Needs verification
    END as portage_type
FROM potential_links p
WHERE 
    -- Rule 1: Always allow if < 500m
    dist <= 500
    OR 
    -- Rule 2: Allow up to 1000m IF a public trail is nearby
    (dist > 500 AND EXISTS (
        SELECT 1 FROM portage_trails t
        WHERE ST_Intersects(t.geom, ST_Buffer(p.geom, 50)) -- Trail must be within 50m of direct line
    ));

-- 5F. Prune Triangles (Logic remains same)
WITH triangle_check AS (
    ...
    FROM valid_portages p1
    JOIN valid_portages p2 ON p1.from_lake = p2.from_lake
    JOIN valid_portages p3 ON p2.to_lake = p3.from_lake AND p3.to_lake = p1.to_lake
)
DELETE FROM valid_portages p
USING triangle_check tc
WHERE p.from_lake = tc.A AND p.to_lake = tc.C
AND tc.dist_via_B < (3 * tc.dist_direct);

-- Insert Final Portages
INSERT INTO candidate_objects (osm_id, source_type, type, name, geom, is_virtual)
SELECT 999900 + row_number() over(), 'system', 'connector', portage_type, geom, TRUE
FROM valid_portages;

\echo '  ✓ Smart connectors created (Road-Aware)'
...
FROM candidate_objects WHERE type = 'connector' AND is_virtual = TRUE
UNION ALL
