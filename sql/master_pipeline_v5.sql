-- =====================================================================
-- AHTI PADDLE MAP PIPELINE - MASTER SCRIPT (V9 - CLEANED)
-- =====================================================================

\echo '╔════════════════════════════════════════════════════════════════╗'
\echo '║  AHTI PADDLE MAP PIPELINE - MASTER SCRIPT v9.0               ║'
\echo '╚════════════════════════════════════════════════════════════════╝'
\echo ''
...
CREATE TABLE candidate_objects (
    ...
    is_narrow BOOLEAN DEFAULT FALSE
);
...
-- =====================================================================
-- STEP 2: EXTRACT CANDIDATES (Relaxed Stream Filter)
-- =====================================================================
\echo '[2/12] Extracting Candidates...'

-- Extract Rivers & Streams
-- FIX: We allow streams with NULL width (assume they are paddleable/connectors)
-- OR streams that are explicitly >= 2m wide.
...
AND (
    ...
    OR (
        ...
        AND (
            width IS NULL 
            OR COALESCE(NULLIF(regexp_replace(width, '[^0-9.]', '', 'g'), '')::numeric, 0) >= 2
        )
    )
)
AND ST_Length(way) > 50;

-- Extract Lakes (2ha Filter)
INSERT INTO candidate_objects (osm_id, source_type, type, name, geom)
SELECT osm_id, 'osm_polygon', 'lake', name, way
FROM planet_osm_polygon
WHERE ("natural" = 'water' OR waterway = 'riverbank')
AND way && ST_MakeEnvelope(:min_x, :min_y, :max_x, :max_y, 3857)
AND ST_Area(way) > 20000; -- 2 Hectares
...
-- =====================================================================
-- STEP 5: V9 TOPOLOGY (Point-to-Point, No Ghost Lines)
-- =====================================================================
\echo '[5/12] Creating v9 Topology...'

DELETE FROM candidate_objects WHERE is_virtual = TRUE;

-- 1. CLASSIFY NARROW LAKES (Anti-Parallel Logic)
UPDATE candidate_objects SET is_narrow = FALSE WHERE type = 'lake';
UPDATE candidate_objects SET is_narrow = TRUE 
WHERE type = 'lake' AND ST_IsEmpty(ST_Buffer(geom, -10));

-- 2. IDENTIFY PORTS
-- We grab the exact point where the river touches the lake.
...
-- 3. GENERATE ROUTES (Point-to-Point)
-- Only draws a line if a lake has TWO rivers (Entry + Exit)
WITH potential_routes AS (
    SELECT 
        p1.lake_id,
        ST_MakeLine(p1.port_pt, p2.port_pt) as geom,
        ST_Distance(p1.port_pt, p2.port_pt) as dist
    FROM lake_ports p1
    JOIN lake_ports p2 ON p1.lake_id = p2.lake_id AND p1.river_id < p2.river_id
    WHERE p1.is_narrow = FALSE
)
...
WHERE dist > 50; -- CHANGED: Reduced from 150m to 50m to allow shorter valid routes

\echo '  ✓ Lake Routes generated'

-- 4. CREATE PORTAGES (Lake-to-Lake)
...
AND ST_DWithin(l1.geom, l2.geom, 400);
...
USING (
    ...
    WHERE (p2.dist + p3.dist) < (3 * p1.dist)
) bad
...
WHERE (tourism IN ('camp_site','lean_to','picnic_site','wilderness_hut') OR amenity IN ('shelter') OR leisure = 'slipway')
AND way && ST_MakeEnvelope(:min_x, :min_y, :max_x, :max_y, 3857)
UNION ALL
SELECT ST_Centroid(way), 'shelter' FROM planet_osm_polygon 
...
SELECT osm_id, ST_MakeValid(way) as geom,
    ...
    END as type
FROM planet_osm_polygon
WHERE ((landuse IN ('forest', 'wood', 'farmland', 'farm', 'meadow', 'grass', 'orchard', 'residential', 'industrial', 'commercial', 'retail') 
...
SET fun_score = CASE 
    ...
    ELSE (
        ...
        CASE WHEN land_type = 'forest' THEN 15 
             ...
             WHEN land_type = 'urban' THEN -20 ELSE 0 END +
...
SELECT 
    network_id,
    (SELECT name FROM paddling_segments s2 WHERE s2.network_id = s.network_id AND name NOT LIKE 'Unnamed%' AND type = 'river' ORDER BY length_m DESC LIMIT 1) as area_name,
