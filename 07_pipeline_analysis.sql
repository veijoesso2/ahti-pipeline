-- =================================================================
-- AHTI PIPELINE: CORE ANALYSIS & SCORING
-- Consolidates Context Import, Segmentation, Enrichment, and Scoring
-- =================================================================
...
-- =================================================================
-- 2. IMPORT CONTEXT DATA (Routes, Rapids, POIs, Land Cover)
-- =================================================================

-- A. Official Canoe Routes (Strict)
...
-- C. POIs (Shelters, Piers, Access Points)
...
WHERE "tourism" IN ('camp_site','lean_to','picnic_site','wilderness_hut') 
   OR "man_made" IN ('pier', 'mast') 
   OR "leisure" = 'slipway'
   OR "waterway" = 'access_point'
UNION ALL
...
-- D. Land Cover (Expanded & Validated)
...
-- =================================================================
-- 4. ENRICHMENT (Spatial Joins & Overlap)
-- =================================================================
...
-- Percentage-Based Land Cover (The Fix for "Mixed")
WITH land_stats AS (
    SELECT 
        s.seg_id,
        lc.type,
...
WHERE s.seg_id = ls.seg_id
  AND ls.type_area > (ls.total_buffer_area * 0.5); -- Must cover >50% of surroundings
...
-- Neighbor Propagation (Smooth Feasibility Gaps)
CREATE TABLE neighbor_stats AS
